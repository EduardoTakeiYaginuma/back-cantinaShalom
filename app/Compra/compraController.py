...
class CompraController:
    ...
    def get_compras():
        ...
        return jsonify([{'id': c.id, 'total': c.total, 'data': c.data, 'usuario_id': c.usuario_id} for c in compras])

    @staticmethod
    def get_compra(compra_id):
        compra = CompraService.get_compra_by_id(compra_id)
        if compra:
            return jsonify({'id': compra.id, 'total': compra.total, 'data': compra.data, 'usuario_id': compra.usuario_id})
    ...
    def create_compra():
        ...
        return jsonify({'id': compra.id, 'total': compra.total, 'data': compra.data, 'usuario_id': compra.usuario_id}), 201
